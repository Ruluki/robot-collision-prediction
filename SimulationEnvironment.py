...
class SimulationEnvironment:
    ...
    #     pm_botPos = (pm_botPos[0], self.screen.get_height() - pm_botPos[1])  # 反轉 Y 座標

    #     img_pos, img_ori = pm_botPos, degrees(self.robot.body.angle)
    #     self._apply_image_to_robot(img_pos, img_ori)

    #     # # 繪製速度向量
    #     # if velocity:
    #     #     velocity_vector = pm_botPos + self.pm2pgV(self.robot.body.velocity) * 5
    #     #     velocity_vector = (velocity_vector[0], self.screen.get_height() - velocity_vector[1])
    #     #     pg.draw.line(self.screen, RED, pm_botPos, velocity_vector)

    #     # 繪製轉向向量
    #     # 獲取並轉換機器人的位置
    #     if steering:
    #         direction = self.pm2pgV(self.steering_force)
    #         magnitude = (direction[0]**2 + direction[1]**2)**0.5
    #         normalized_direction = (direction[0] / magnitude, direction[1] / magnitude)
    #         steering_vector = (pm_botPos[0] + normalized_direction[0] * 150, pm_botPos[1] + normalized_direction[1] * 150)

    #         # 轉換 steering_vector 的座標
    #         steering_vector = (steering_vector[0], self.screen.get_height() - steering_vector[1])

    #         # 計算線段的實際長度
    #         actual_length = ((steering_vector[0] - pm_botPos[0])**2 + (steering_vector[1] - pm_botPos[1])**2)**0.5
            
    #         if actual_length > 150:
    #             # 縮放方向向量到長度150
    #             scaling_factor = 150 / actual_length
    #             steering_vector = (pm_botPos[0] + (steering_vector[0] - pm_botPos[0]) * scaling_factor, 
    #                             pm_botPos[1] + (steering_vector[1] - pm_botPos[1]) * scaling_factor)
    ...
    def _detect_collisions(self):
        for wall_shape in self.wall_shapes:
            ...
            if collisions.points:
                # 反轉機器人位置並繪製碰撞圖像
                img_pos, img_ori = self.pm2pgP(self.robot.body.position), degrees(self.robot.body.angle)
                # img_pos = (img_pos[0], self.screen_height - img_pos[1])  # 反轉 Y 座標
