...
class Wander():
    ...
    def get_action(self, timestep_i, current_orientation, actions_checked=[]):
        ...
        perlin_noise = self.perlin_noise.noise( (float(timestep_i*self.action_repeat)+self.offset0) / self.scale0 )
        perlin_noise += self.perlin_noise.noise( (float(timestep_i*self.action_repeat)+self.offset1) / self.scale1 )
